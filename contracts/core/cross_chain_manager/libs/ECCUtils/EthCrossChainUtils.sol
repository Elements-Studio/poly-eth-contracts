...
library ECCUtils {
    ...
    */
    ...
    function containMAddresses(address[] memory _validators, address[] memory _signers, uint _m) internal pure returns (bool){
        uint m = 0;
        for(uint i = 0; i < _signers.length; i++){
            for (uint j = 0; j < _validators.length; j++) {
                if (_signers[i] == _validators[j]) {
    ...
    function decodeTxParam(bytes memory rawParam) internal pure returns(TxParam memory param) {
        (param.txHash, 
        param.crossChainId,
        param.fromContract,
        param.toChainId,
    ...
    */
    ...
    function checkNodeHash(bytes memory raw,uint offset,bytes memory hash) internal pure returns(bool flag) {
        ...
        assembly {
            switch gt(size,0x20) // for value less than 32 bytes , hash(value) = value
            case 0 {
                flag := eq(mload(add(raw,offset)),mload(add(hash,0x20)))
            }
            case 1 {
                flag := eq(keccak256(add(raw,offset),add(size,sub(offset_,offset))),mload(add(hash,0x20)))
            }
        }
    }
    ...
    // won't change memory
    function rlpReadKind(bytes memory raw, uint offset) internal pure returns(Kind kind ,uint size ,uint offset_) {
        assembly {
